...
class PLTTrainer(pl.core.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        stensor = ME.SparseTensor(coordinates=batch[self.coords_name].int(), features=batch[self.features_name])
        out = self.model(stensor).F
        labels = batch[self.labels_name].long()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        labels = batch[self.labels_name].long().cpu()
...
class PLTTester(pl.core.LightningModule):
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if self.save_predictions:
            coords = batch[self.coords_name].cpu()
            labels = batch[self.labels_name].cpu()
            preds = preds.cpu()
            conf = conf.cpu()
        ...
        self.test_step_outputs.append({"iou": iou})
        return {'iou': iou}
    ...
    def on_test_epoch_end(self):
        ...
        for return_dict in self.test_step_outputs:
        ...
        os.makedirs(os.path.join(self.trainer.default_root_dir, 'results'), exist_ok=True)
        csv_columns = list(results.keys())

        _, ckpt_name = os.path.split(self.checkpoint_path)
        ckpt_name = ckpt_name[:-5]

        csv_file = os.path.join(self.trainer.default_root_dir, 'results', ckpt_name+'_test.csv')
